    ...
    <div class="container mx-auto p-10">
        ...
        <div class="overflow-x-auto rounded-box bg-white p-5 shadow-xs">
            <table class="table">
                ...
                <tbody>
                    @forelse ($ticket_types as $index => $ticket_type)
                        <tr>
                            <th>{{ $index + 1 }}</th>
                            <td>{{ $ticket_type->name }}</td>
                            <td>
                                <button class="btn btn-sm btn-primary mr-2" onclick="openEditModal(this)" data-id="{{ $ticket_type->id }}" data-nama="{{ $ticket_type->name }}">Edit</button>
                                <button class="btn btn-sm bg-red-500 text-white" onclick="openDeleteModal(this)" data-id="{{ $ticket_type->id }}">Hapus</button>
                            </td>
                        </tr>
                        @empty
                        <tr>
    ...
    <dialog id="add_modal" class="modal">
        <form method="POST" action="{{ route('admin.ticket-types.store') }}" class="modal-box">
            ...
            <div class="form-control w-full mb-4">
                ...
                <input type="text" placeholder="Masukkan nama tipe tiket" class="input input-bordered w-full" name="name" required />
            </div>
     ...
     <dialog id="edit_modal" class="modal">
        <form method="POST" class="modal-box">
            ...
            <div class="form-control w-full mb-4">
                ...
                <input type="text" placeholder="Masukkan nama tipe tiket" class="input input-bordered w-full" value="Kategori Contoh" id="edit_category_name" name="name" />
            </div>
            <div class="modal-action">
                <button class="btn btn-primary" type="submit">Simpan</button>
